...
app.get('/api/challenge/today', async (req, res) => {
  try {
    ...
    const result = await db.query(
      'SELECT date, video_id, freeze_frame, answer, standing_pins FROM daily_challenges WHERE date = $1',
      [today]
    );
    ...
    res.json({
      ...
      freezeFrame: challenge.freeze_frame,
      // answer and standingPins should only be sent after guess verification
    });
  } catch (error) {
